...
public class InfoChangeView extends JFrame implements ActionListener {
    ...
    JFormattedTextField birthField = new JFormattedTextField( new NumberFormatter( new DecimalFormat( "####-##-##" ) ) );
    
    public JFormattedTextField getBirthField() {
        birthField.setBounds( 100, 100, 30, 20 );
        
        birthField.addKeyListener( new KeyAdapter() {
            @Override
            public void keyTyped( KeyEvent e ) {
                
                if ( birthField.getText().length() <= 10 ) {
                    e.consume();
                }
